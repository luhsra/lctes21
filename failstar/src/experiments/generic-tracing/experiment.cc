...
void  GenericTracing::parseOptions() {
	...
	CommandLine::option_handle FULL_TRACE = cmd.addOption("", "full-trace", Arg::None,
		"--full-trace \tDo a full trace for every mem event (more data, default: off)");
	CommandLine::option_handle FULL_TRACE_IPS = cmd.addOption("", "full-trace-ips", Arg::None,
		"--full-trace-ips \tDo a full trace for every ip event (more data, default: off)");
	CommandLine::option_handle FULL_TRACE_EVERY = cmd.addOption("", "full-trace-every", Arg::None,
		"--full-trace-every \tDo a full trace for every ip and mem event (more data, default: off)");
	...
	if (cmd[FULL_TRACE_IPS]) {
		this->full_trace_ips = true;
	}
	if (cmd[FULL_TRACE_EVERY]) {
		this->full_trace = true;
		this->full_trace_ips = true;
	}
	...
	m_log << "full-trace-ips: "	  << this->full_trace_ips << std::endl;
}
...
bool GenericTracing::run()
{
	...
	tp.setFullTraceIps(this->full_trace_ips);
